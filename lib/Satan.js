...
Satan.waitForDaemon = function(fn) {
  process.on("exit", function() {
    Satan.killDaemon();
  });
  process.on("SIGTERM", function() {
  ...
  Satan.client.call('waitForDaemon', function(err, res) {
    fn(err, res);
  });
};
...
Satan.launchDaemon = function(cb) {
  ...
  if (process.env.PM2_SOCKET_ADDR) {
    env.PM2_SOCKET_ADDR = process.env.PM2_SOCKET_ADDR;
  }
